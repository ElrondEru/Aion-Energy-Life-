...
public class GatherableController extends VisibleObjectController<Gatherable>
{
	...
	public void onStartUse(final Player player)
	{
		...
		{		
			...
			int gatherRate = 2; // 1x rates (probably make config later, if fixed to non-linear statistic probability)
			float maxrate = 0;
			int rate = 0;
	...
	public void rewardPlayer(Player player)
	{
		if(player != null)
		{
			...
			int xpReward = (int)((0.0031*(skillLvl+5.3)*(skillLvl+1592.8)+60)*player.getRates().getGatheringXPRate());
