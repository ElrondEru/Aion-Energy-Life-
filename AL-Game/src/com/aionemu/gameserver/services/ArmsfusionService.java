 ...
 * 
 * @author zdead @ contribuitor bluecrime
 * 
 */
public class ArmsfusionService
{
	...
	public static void fusionWeapons(Player player, int firstItemUniqueId, int secondItemUniqueId, int price)
	{
		...
		/*
		 * Fused weapon cannot be recombined
		 */
		if(firstItem.hasFusionedItem() || secondItem.hasFusionedItem())
		{
			return;
		}
	...
	public static void breakWeapons(Player player, int weaponToBreakUniqueId)
	{
		...
		/*
		 * Remove all extra manastones
		 */
		int count = 0;
		Set<ManaStone> manaStones = weaponToBreak.getItemStones();
		for(ManaStone stone : manaStones)
		{
			if(count > weaponToBreak.getItemTemplate().getItemSlot())
				ItemService.removeManastone(player, weaponToBreak.getObjectId(), stone.getSlot());
			count++;
		}
