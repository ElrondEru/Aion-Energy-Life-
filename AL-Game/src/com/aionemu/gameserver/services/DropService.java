	...
	public void requestDropItem(Player player, int npcId, int itemIndex)
	{
		...
		synchronized(dropItems)
		{
			for(DropItem dropItem : dropItems)
			{
				...
				{
					if(ItemService.newItem(dropItem.getDropTemplate ().getItemId(),0).getItemTemplate().getItemQuality()==ItemQuality.UNIQUE || ItemService.newItem(dropItem.getDropTemplate ().getItemId(),0).getItemTemplate().getItemQuality()==ItemQuality.EPIC || ItemService.newItem(dropItem.getDropTemplate ().getItemId(),0).getItemTemplate().getItemQuality()==ItemQuality.LEGEND)
					{
					  Iterator<Player> iter = World.getInstance().getPlayersIterator();
						while(iter.hasNext()){
							PacketSendUtility.sendMessage(iter.next(), player.getCommonData().getName() + " A Looter " + "[item:" + dropItem.getDropTemplate ().getItemId() + ";ver1;;]");
					  }
					}
